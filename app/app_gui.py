...
# 🧭  NAVEGADOR DE EJEMPLOS
# ═══════════════════════════════════════════════
elif modulo == "🧭  Navegador de Ejemplos":
    st.header("🧭 Navegador de Ejemplos")
    st.write(
        "Explora los archivos sintéticos paso a paso. Esta vista ayuda a entender "
        "qué cambia al leer, limpiar, agrupar y revisar un Excel."
    )

    ejemplos_dir = Path(__file__).resolve().parent.parent / "ejemplos"
    archivos = sorted(
        archivo for archivo in ejemplos_dir.glob("*.xlsx")
        if not archivo.name.startswith(("reporte_", "validacion_"))
    )

    if not archivos:
        st.info("No hay ejemplos disponibles. Ejecuta los generadores de ejemplos primero.")
    else:
        nombres = [archivo.name for archivo in archivos]
        seleccionado = st.selectbox("Selecciona un ejemplo", nombres)
        ruta = ejemplos_dir / seleccionado

        try:
            df = pd.read_excel(ruta)
            st.caption(f"Archivo: {ruta.name} · {ruta.stat().st_size:,} bytes")

            metricas = st.columns(4)
            metricas[0].metric("Filas", f"{len(df):,}")
            metricas[1].metric("Columnas", f"{len(df.columns):,}")
            metricas[2].metric("Celdas vacías", f"{int(df.isna().sum().sum()):,}")
            metricas[3].metric("Memoria", f"{df.memory_usage(deep=True).sum() / 1024:.1f} KB")

            pestañas = st.tabs(["Vista previa", "Tipos y nulos", "Visualización"])
            with pestañas[0]:
            ...
            with pestañas[1]:
                resumen = pd.DataFrame({
                    ...
                    "nulos": [int(valor) for valor in df.isna().sum()],
                    "no_nulos": [int(valor) for valor in df.notna().sum()],
                })
                st.dataframe(resumen, use_container_width=True, hide_index=True)
            with pestañas[2]:
                numericas = df.select_dtypes(include="number")
                if numericas.empty:
                    st.info("Este ejemplo no tiene columnas numéricas para graficar.")
                else:
                    columna = st.selectbox("Columna numérica", list(numericas.columns))
                    st.bar_chart(df[columna].reset_index(drop=True))
                    st.caption("El gráfico muestra los valores en el orden original del Excel.")
        except Exception as error:
            st.error(f"No se pudo leer el ejemplo: {error}")
